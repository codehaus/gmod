...
class ProxyMetaClassFromClosureTests extends AbstractProxyMetaClassTestCase {
   ...
   def proxyFromFooWithPropertyMissing_set() {
      def map = [foo:"Foo"]
      def impl = [:]
      impl[key('propertyMissing',[String])] = { String name -> map[name] }
      impl[key('propertyMissing',[String,Object])] = { String name, value -> map[name] = value }
      proxy( Foo, impl )
   }
   
   private key( String name, types ){
      new ProxyMethodKey( name, types )
   }
}